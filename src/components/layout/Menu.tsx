...
export function Menu({ routes }: MenuProps) {
  ...
  return (
    <>
      ...
      />
      <nav
        className={`${isMenuOpen && 'h-dvh'} fixed top-0 left-0 z-50 flex w-64 flex-col gap-3 ${isMenuOpen ? 'rounded-r-md' : 'rounded-md'} bg-white p-8 text-body-600 shadow-lg transition-transform duration-200 lg:min-h-[calc(100vh-7.5rem)] xl:static xl:flex xl:w-full xl:flex-col xl:p-6 xl:shadow-none ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} xl:pointer-events-auto xl:translate-x-0 xl:opacity-100`}
